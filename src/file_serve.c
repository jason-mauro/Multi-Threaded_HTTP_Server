...
void serve_file(const char* path, const int clientfd) {
    if (strcmp(path, "/") == 0) {
        // Default page
        const char *html_content = "<!DOCTYPE html>\n<html lang=\"en\">\n<body>\n<h1>Hello, World!</h1>\n<p>Welcome to the server. Enter a file name or \"words\" after the URL to get started!</p>\n</body>\n</html>\n";
        const size_t content_length = strlen(html_content);
        char header[256];
        ...
        send(clientfd, html_content, strlen(html_content), 0);
    } else {
        char filepath[512] = {0};
        ...
        if (file == -1){
            const char *file_not_found_response = "HTTP/1.0 404 Not Found\r\n"
                ...
                "<body>\n"
                "    <h1>404 Not Found</h1>\n"
                "    <p>File was unable to be found from the provided file path.</p>\n"
                "</body>\n"
                "   </html>";
